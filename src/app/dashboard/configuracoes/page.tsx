...
import { Settings, MessageSquare, Save, Users, UserCheck } from "lucide-react";

// Tipos de Alertas para Clientes
const alertasClienteTipos = [
  { id: "confirmacaoAgendamento", label: "Confirmação de Agendamento" },
  { id: "lembreteAgendamento24h", label: "Lembrete de Agendamento (24h antes)" },
  { id: "lembreteAgendamento1h", label: "Lembrete de Agendamento (1h antes)" },
  { id: "servicoIniciado", label: "Serviço Iniciado" },
  { id: "servicoFinalizado", label: "Serviço Finalizado / Pronto para Retirada" },
  { id: "orcamentoPronto", label: "Orçamento Pronto para Aprovação" },
  { id: "pesquisaSatisfacao", label: "Pesquisa de Satisfação pós-serviço" },
];

// Tipos de Alertas para Funcionários
const alertasFuncionarioTipos = [
  { id: "novoAgendamentoAtribuido", label: "Novo Agendamento Atribuído ao Mecânico" },
  { id: "veiculoChegouOficina", label: "Veículo do Agendamento Chegou (para mecânico)" },
  { id: "alteracaoOsAtribuida", label: "Alteração em OS Atribuída ao Mecânico" },
  { id: "pecaFaltanteServico", label: "Peça Faltante para Serviço em Andamento" },
];
          ...
          <div className="space-y-4">
            ...
            <div className="grid md:grid-cols-2 gap-x-6 gap-y-4">
              {alertasClienteTipos.map((alerta) => (
                <div key={alerta.id} className="flex items-center justify-between space-x-2 p-3 border rounded-md bg-muted/20">
                  <Label htmlFor={`cliente-${alerta.id}`} className="flex-1 cursor-pointer">
                    {alerta.label}
                  </Label>
                  <Switch
          ...
          <div className="space-y-4">
            ...
            <div className="grid md:grid-cols-2 gap-x-6 gap-y-4">
              {alertasFuncionarioTipos.map((alerta) => (
                <div key={alerta.id} className="flex items-center justify-between space-x-2 p-3 border rounded-md bg-muted/20">
                  <Label htmlFor={`func-${alerta.id}`} className="flex-1 cursor-pointer">
                    {alerta.label}
                  </Label>
                  <Switch
                    id={`func-${alerta.id}`}
